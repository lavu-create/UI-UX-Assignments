// ===== Packages Table Data =====
const packages = [
  { id: 1, destination: "Paris, France", type: "Standard", durationDays: 5, basePrice: 76000, season: "low" },
  { id: 2, destination: "Paris, France", type: "Deluxe", durationDays: 5, basePrice: 150000, season: "mid" },
  { id: 3, destination: "Paris, France", type: "Premium", durationDays: 5, basePrice: 400000, season: "high" },
  { id: 4, destination: "Goa, India", type: "Standard", durationDays: 3, basePrice: 5000, season: "low" },
  { id: 5, destination: "Goa, India", type: "Deluxe", durationDays: 3, basePrice: 1800, season: "mid" },
  { id: 6, destination: "Goa, India", type: "Premium", durationDays: 3, basePrice: 50000, season: "high" },
  { id: 7, destination: "Bali, Indonesia", type: "Standard", durationDays: 7, basePrice: 76000, season: "low" },
  { id: 8, destination: "Bali, Indonesia", type: "Deluxe", durationDays: 7, basePrice: 150000, season: "mid" },
  { id: 9, destination: "Bali, Indonesia", type: "Premium", durationDays: 7, basePrice: 400000, season: "high" },
  { id:10, destination: "Disney Land, USA", type: "Standard", durationDays: 3, basePrice: 140000, season: "low" },
  { id:11, destination: "Disney Land, USA", type: "Deluxe", durationDays: 3, basePrice: 180000, season: "mid" },
  { id:12, destination: "Disney Land, USA", type: "Premium", durationDays: 3, basePrice: 360000, season: "high" }
];
...
// ===== Render packages table dynamically =====
const tableBody = document.querySelector(".table tbody");
if (tableBody) {
  tableBody.innerHTML = "";

  const includesMap = {
    "Paris, France": "Hotel, Breakfast, City Tour",
    "Goa, India": "Resort, Breakfast, Beach Activities",
    "Bali, Indonesia": "Villa, Breakfast, Island Tour",
    "Disney Land, USA": "Hotel, Breakfast, Park Tickets"
  };
  ...
  packages.forEach(pkg => {
    ...
    tr.innerHTML = `
      ...
      <td>$${pkg.basePrice}</td>
      <td>${includesMap[pkg.destination] || ""}</td>
      <td>$${calcFinalPrice(pkg)}</td>
      <td>
        <a class="btn btn--table"
           href="booking.html?dest=${encodeURIComponent(pkg.destination)}&pkg=${encodeURIComponent(pkg.type)}">
          Choose
        </a>
      </td>
    `;
...
if (bookingForm) {
  ...
  // --- Pre-fill booking form from query string ---
  const params = new URLSearchParams(window.location.search);
  const destParam = params.get("dest");
  const pkgParam = params.get("pkg");
  if (destParam) destinationSelect.value = destParam;
  ...
  function calcBookingTotal() {
    ...
    const promoCode = promo ? promo.value.trim().toUpperCase() : "";
    switch (promoCode) {
